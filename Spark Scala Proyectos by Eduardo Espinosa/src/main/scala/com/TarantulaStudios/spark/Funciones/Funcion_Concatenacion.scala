...
object Funcion_Concatenacion extends App {

  val spark = SparkSession.builder()
    .master("local[*]")
    .appName("Funcion Concatenación")
    .getOrCreate()
  ...
  df.select(concat(col("Fabricante"), lit(" "), col("Modelo")))
    .show(false)

  df.withColumn("Modelo Completo", concat(col("Fabricante"), lit(" "), col("Modelo")))
    .drop("Fabricante")
    .drop("Modelo")
    .show(false)
  ...
  df.withColumn("Modelo Completo", concat_ws(" ", col("Fabricante"), col("Modelo")))
    .drop("Fabricante")
    .drop("Modelo")
    .show(false)
  ...
  spark.sql("select Id, CONCAT(Fabricante,' ',Modelo) as Modelo, Estado_Importacion, Precio  from CARROS")
    .show(false)

  spark.sql("select Fabricante ||' '|| Modelo as Modelo from CARROS order by Fabricante")
    .show(false)
}
